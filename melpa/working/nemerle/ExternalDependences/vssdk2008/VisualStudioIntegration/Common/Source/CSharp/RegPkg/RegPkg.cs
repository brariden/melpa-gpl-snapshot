...
namespace Microsoft.VisualStudio.Tools {
    ...
    public static class RegPkg {
        ...
        public static void ProcessAssembly(string fileName, Hive hive, RegPkgContext context, bool register, RegistrationMode mode) {
            ...
            if (Utilities.AlternateAssemblyLoaded(fileName, a))
            {
                WriteWarningLine(string.Format(CultureInfo.InvariantCulture, Resources.RegPkg_AssemblyLoad, fileName, a.Location));
            }
        ...
        /// <devdoc>
        ///     Console wrapper that outputs warnings to the console, but only if
        ///     we were invoked through Main.
        /// </devdoc>
        internal static void WriteWarningLine(string line)
        {
            WriteLine(string.Format(CultureInfo.InvariantCulture, Resources.RegPkg_Warning, line));
        }
