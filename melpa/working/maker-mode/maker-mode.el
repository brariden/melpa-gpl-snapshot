;;; maker-mode.el --- Emacs mode for maker (scala build tool) 

;; Copyright (C) 2014  Sam Halliday

;; Author: Sam Halliday <Sam.Halliday@gmail.com>
;; Keywords: processes, tools
;; Package-Requires: ((s "1.3.0") (dash "2.8.0"))
;; URL: https://github.com/fommil/maker-mode
...
(defun maker:initialize-for-compilation-mode ()
  ...
  (setq-local
   ...
   '(("^\\.*\\[error\\][[:space:]]\\([^[:space:]:]+\\):\\([[:digit:]]+\\):" 1 2 nil 2 1)
     ("^\\.*\\[warn\\][[:space:]]\\([^[:space:]:]+\\):\\([[:digit:]]+\\):" 1 2 nil 1 1)
     ("^\\.*\\[info\\][[:space:]]\\([^[:space:]:]+\\):\\([[:digit:]]+\\):" 1 2 nil 0 1)))
  (setq-local
